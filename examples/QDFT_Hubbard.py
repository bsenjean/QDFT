...
code_directory = os.getenv('QDFT_DIR')
# ===========================================================#
...
# ===========================================================#
# =============== End of the input initialization =================#
# ===========================================================#
'''Initializing simulation inputs to QDFT'''
backend, blocking, allowed_increase, weights, potential, n_sites, v = QDFT.simulation(simulation, weights_choice, nshots, Hubbard,
                                                               n_elec, n_occ, n_qubits, prob_1, prob_2, potential)
...
for U in U_list:
    ...
    for SCF_ITER in range(1, SCF_maxiter + 1):
        ...
        eigvecs = np.array(Operator(result.eigenstates).transpose())
        ...
        if nshots is not False:
            energies = [QDFT.sampled_expectation_value(bounds[state], H_qubit, backend, simulation, nshots=nshots) for state
                        in range(n_occ)]
        else:
